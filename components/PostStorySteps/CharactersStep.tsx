...
export const CharactersStep = ({ control, formState: { errors }, getValues, setValue }) => {
    ...
    return (
        <div className={styles.inputs}>
            <FormSearchInput error={errors?.characters?.message} autoFocus fullWidth onResultItemSelect={onCharacterSelect} control={control} wrapClassName={styles.searchInputWrap} entity='character'
                             label='Characters' name='characters'/>
            <FormSearchInput error={errors?.fandoms?.message} onResultItemRemove={onFandomRemove} control={control} fullWidth wrapClassName={styles.searchInputWrap} entity='fandom' label='Fandoms'
                             name='fandoms'/>
        </div>
    );
};
