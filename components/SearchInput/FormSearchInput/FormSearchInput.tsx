...
export const FormSearchInput = ({ control,  error, onResultItemSelect, onResultItemRemove, name, ...props }: FormSearchInputProps) => {
    ...
    const renderInput = ({ field }) => <SearchInput error={error} onBlur={field.onBlur} selectedValues={field.value} onResultSelect={onChange(field)} onResultRemove={onChange(field, false)} {...props} />;
    return (
